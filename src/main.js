...
class EffectCanvas {
	...
	constructor(width, height, loader) {
		...
		shaderData.uniforms.uTextureDropShine.value =
			loader.resources["https://stefanweck.nl/codepen/shine.png"].texture;
		shaderData.uniforms.uTextureBackground.value =
			loader.resources["bg"].texture;
		shaderData.uniforms.uTextureForeground.value =
			loader.resources["fg"].texture;
		shaderData.uniforms.vTextureSize.value = [
			loader.resources["bg"].texture.width,
			loader.resources["bg"].texture.height
		];
...
class DropletManager {
	...
	constructor(stage, loader) {
		...
		this.dropletSmallTexture =
			loader.resources["https://stefanweck.nl/codepen/alpha.png"].texture;
	...
	checkLargeToSmallDropletCollision(droplet) {
		...
		for (let i = 0; i < arrayIndexes.length; i++) {
			...
			// Store the length of the array so the for loop doesn't have to do that every run
			const smallDropletsLength = this.smallDroplets[arrayIndexes[i].x][
				arrayIndexes[i].y
			].length;
